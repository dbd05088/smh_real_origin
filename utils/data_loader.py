...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img_path = os.path.join("dataset", str(self.dataset), str(img_name))
...
def get_train_datalist(args, cur_iter: int) -> List:
    if args.mode == "joint":
        ...
        labeled_collection_name = get_train_collection_name(
            dataset=args.dataset,
            exp=args.exp_name,
            rnd=args.rnd_seed,
            n_cls=args.n_cls_a_task,
        ...
        unlabeled_collection_name = get_train_collection_name(
            dataset=args.dataset,
            exp=args.exp_name,
            rnd=args.rnd_seed,
            n_cls=args.n_cls_a_task,
        ...
        labeled_datalist = pd.read_json(
            f"collections/{args.dataset}/{labeled_collection_name}.json"
        ).to_dict(orient="records")

        unlabeled_datalist = pd.read_json(
            f"collections/{args.dataset}/{unlabeled_collection_name}.json"
        ).to_dict(orient="records")

        logger.info(f"[Train] Get datalist from {collection_name}.json")

    return (labeled_datalist, unlabeled_datalist)


def get_train_collection_name(dataset, exp, rnd, n_cls, iter, label):
    collection_name = "{dataset}_train_{exp}_rand{rnd}_cls{n_cls}_task{iter}_{label}".format(
        dataset=dataset, exp=exp, rnd=rnd, n_cls=n_cls, iter=iter, label=label
    )
