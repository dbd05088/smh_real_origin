    ...
    def train(self, cur_iter, n_epoch, batch_size, n_worker):
        ...
        for epoch in range(n_epoch):
            ...
            for i, data in enumerate(train_loader):
                ...
                assert old_cls_index.size(
                    0) + new_cls_index.size(0) == y.size(0)
                ...
                if new_cls_index.size(0) > 0:
                    ...
                    if do_cutmix:
                        ...
                        cls_loss += self.criterion(g[new_cls_index],
                                                   y[new_cls_index])
            ...
            writer.add_scalar(
                f"task{cur_iter}/train/cls_loss", total_cls_loss /
                num_batches, epoch
            )
            writer.add_scalar(
                f"task{cur_iter}/train/distilling", total_dist_loss /
                num_batches, epoch
            )
            writer.add_scalar(
    ...
    def icarl_evaluation(self, eval_loader):
        ...
        with torch.no_grad():
            for data in eval_loader:
                ...
                total_correct += torch.sum(pred.detach().cpu()
                                           == y.unsqueeze(1)).item()
